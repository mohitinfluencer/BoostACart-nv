...
export default function UsageMeter({
  ...
  const usage = Math.min((leadsThisMonth / maxLeads) * 100, 100)
  const remaining = Math.max(maxLeads - leadsThisMonth, 0)
    ...
    <div className="space-y-3">
      ...
      <div className="flex justify-between items-center text-sm">
        <span className={`font-semibold ${textColorClass}`}>
          {leadsThisMonth} / {maxLeads} leads used
        </span>
        <span className="text-gray-400">{remaining} remaining this month</span>
      </div>
