...
export default function WidgetPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (store.maxLeads !== null && store.remainingLeads <= 0) {
    ...
    try {
      ...
      const { error: updateError } = await supabase
        .from("stores")
        .update({
          total_leads: store.remainingLeads + (store.maxLeads !== null ? (store.maxLeads - store.remainingLeads) + 1 : 1),
          leads_this_month: store.maxLeads !== null ? store.maxLeads - store.remainingLeads + 1 : 1,
